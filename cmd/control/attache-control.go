...
func main() {
	...
	for range ticks {
		...
		go func() {
			<-catchSignals
			logger.Error("interrupted, cleaning up session")
		...
		if nodeHasLock {
			...
			if len(nodesInDest) == 0 {
				...
				// service.
				nodesMissing := conf.RedisPrimaryCount - len(nodesInAwait)
				if nodesMissing <= 0 {
					logger.Infof("creating new cluster with nodes %q", nodesInAwait[0:conf.RedisPrimaryCount])

					err := control.RedisCLICreateCluster(nodesInAwait[0:conf.RedisPrimaryCount])
					if err != nil {
						close(doneChan)
						session.Cleanup()
						logger.Fatal(err)
			...
			} else if len(nodesInDest) < conf.RedisPrimaryCount {
				...
				if err != nil {
					logger.Info("issue encountered, releasing lock")
					close(doneChan)
					session.Cleanup()
					logger.Fatal(err)
				} else {
